...
from gecko.train.train_utils import (
    load_images,
    load_json_data,
)
from gecko.train.conversation import SeparatorStyle
...
class ChatDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.conv.sep_style == SeparatorStyle.PLAIN:
            ...
            keyword_str = None
            encoding = self.processor(conv_str, keyword_str, sub_images, return_tensors="pt", truncation=True, max_length=self.max_seq_len)
        else:
            self.conv.messages = conv_messages
            conv_str = self.conv.get_prompt()
            keyword_str = self.conv.get_keyword_prompt()
            encoding = self.processor(conv_str, keyword_str, sub_images, return_tensors="pt", truncation=True, max_length=self.max_seq_len)
        ...
        elif self.conv.sep_style == SeparatorStyle.SINGLE or self.conv.sep_style == SeparatorStyle.LLAMA_3 or self.conv.sep_style == SeparatorStyle.PHI_3:
            sep_id = self.processor.tokenizer.convert_tokens_to_ids(self.conv.sep)
            sep_idxs = torch.nonzero((input_ids == sep_id), as_tuple=True)[0].tolist()
