...
# class CropImageProcessor(ImageProcessingMixin):

#     def __init__(self, model_name, crop_size=336, **kwargs):
#         self.processor = AutoProcessor.from_pretrained(model_name).image_processor
#         self.crop_size = crop_size if crop_size is not None else int(re.search(r'\d+$', model_name).group())
...
#         for image in images:
#             outputs, output_coords = self.split_image(image)
...
#         Returns:
#             List[torch.Tensor]:
#                 List of pixel values of the cropped images. Has shape (num_patches, num_channels, height, width)
#             List[List[int]]:
#                 Coordinates of the cropped images. Has shape (num_patches, 2)"""


#         width, height = image.size
#         crop_size = stride = self.crop_size

#         x_steps = math.ceil(width / stride)
#         y_steps = math.ceil(height / stride)

#         x_coords = []
#         y_coords = []
#         for i in range(x_steps):
#             x_coords.append([i*stride, min((i+1)*stride, width)])
#         for i in range(y_steps):
#             y_coords.append([i*stride, min((i+1)*stride, height)])


#         outputs = [image]
#         output_coords = [[width / 2, height / 2]]
#         for i, x in enumerate(x_coords):
#             for j, y in enumerate(y_coords):
#                 output_coords.append([i, j])
#                 outputs.append(image.crop((x[0], y[0], x[1], y[1])))

#         outputs = self.processor(images=outputs, return_tensors="pt")["pixel_values"]
...
class GeckoProcessor(ProcessorMixin):
    ...
    def __init__(self, image_processor=None, tokenizer=None, use_keyword=False, crop_size=336, pe_strategy='size_coor', **kwargs):
        super().__init__(image_processor, tokenizer)
        self.crop_size = crop_size if crop_size is not None else int(image_processor.crop_size['height'])
    ...
    def split_image(self, image: Image.Image):
        ...
        outputs = [image]
        output_coords = []
        if self.pe_strategy == 'size_coor':
            output_coords.append([0, 0, 1, 1])
            for i, x in enumerate(x_coords):
                for j, y in enumerate(y_coords):
                    output_coords.append([x[0] / width, y[0] / height, x[1] / width, y[1] / height])
                    outputs.append(image.crop((x[0], y[0], x[1], y[1])))
        elif self.pe_strategy == 'abs_coor':
            output_coords.append([0, 0])
            for i, x in enumerate(x_coords):
                for j, y in enumerate(y_coords):
                    output_coords.append([i, j])
                    outputs.append(image.crop((x[0], y[0], x[1], y[1])))
        elif self.pe_strategy == 'norm_coor':
            output_coords.append([0.5, 0.5])
            for i, x in enumerate(x_coords):
                for j, y in enumerate(y_coords):
                    output_coords.append([(x[0] + x[1]) / 2 / width, (y[0] + y[1]) / 2 / height])
                    outputs.append(image.crop((x[0], y[0], x[1], y[1])))
        else:
            raise ValueError(f"Invalid positional encoding strategy: {self.pe_strategy}")
        outputs = self.image_processor(images=outputs, return_tensors="pt")["pixel_values"]
        output_coords = torch.tensor(output_coords)
    ...
    def __call__(
        ...
        return BatchFeature(data={**text_inputs, **image_inputs, "keyword_input_ids": keyword_inputs})
